...
class WarningsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final warningColor =
        Color((properties['warningColor'] as int?) ?? 0xFFFFAA00);
            ...
            children: warnings.isEmpty
                ? [
                    Icon(Icons.check_circle,
                        color: const Color(0xFF00CC66), size: iconSize),
                    Text('OK',
                        style: TextStyle(
                ...
                : warnings
                    .map((w) => _WarningIcon(
                          ...
                          color:
                              _colorFor(w.type, color, warningColor, infoColor),
                          size: iconSize,
                          tooltip: w.message,
                          label: showLabels ? w.message : null,
                        ))
                    .toList(),
          ),
        ),
  ...
  List<_Warning> _parseWarnings(String raw) {
    ...
    return raw
        ...
        .map((s) {
          ...
          final msg =
              parts.length > 1 ? parts.sublist(1).join(':').trim() : type;
          return _Warning(type, msg);
        })
        .where((w) => w.type.isNotEmpty)
        .toList();
  }
  ...
  Color _colorFor(String type, Color error, Color warning, Color info) {
    if (type == 'fault' ||
        type == 'error' ||
        type == 'motor' ||
        type == 'brake') return error;
    if (type == 'temp' ||
        type == 'battery' ||
        type == 'oil' ||
        type == 'engine') return warning;
      ...
      child: label == null
          ...
          : Column(
              ...
              children: [
                ...
                Text(
                  label!,
                  style: TextStyle(color: color, fontSize: size * 0.32),
                ),
              ],
            ),
    );
