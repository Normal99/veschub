...
void main() {
  ...
  group('Tier 1: PropertyCategory Enum & Schema Coverage', () {
    test('T1.1: PropertyCategory enum defines 4 distinct UI categories', () {
      ...
      expect(
          PropertyCategory.layoutAndSpacing.label, equals('Layout & Spacing'));
      expect(PropertyCategory.fontsAndColors.label, equals('Fonts & Colors'));
    });

    test(
        'T1.2: Every core widget manifest property maps to a valid PropertyCategory',
        () {
      for (final kind in coreKinds) {
        final props = propertyManifest[kind];
        expect(props, isNotNull,
            reason: 'Manifest missing for widget kind $kind');
        expect(props, isNotEmpty,
            reason: 'Manifest empty for widget kind $kind');

        for (final meta in props!) {
          expect(meta.category, isA<PropertyCategory>(),
              reason:
                  'Property "${meta.key}" in widget kind "$kind" missing category');
        }
      }
    });

    test(
        'T1.3: categorizedProperties groups gauge manifest correctly across all 4 categories',
        () {
      final categorized = categorizedProperties('gauge');
      expect(categorized.keys.length, equals(4));

      final visualKeys =
          categorized[PropertyCategory.visuals]!.map((m) => m.key).toList();
      final bindingKeys = categorized[PropertyCategory.dataBindings]!
          .map((m) => m.key)
          .toList();
      final layoutKeys = categorized[PropertyCategory.layoutAndSpacing]!
          .map((m) => m.key)
          .toList();
      final fontColorKeys = categorized[PropertyCategory.fontsAndColors]!
          .map((m) => m.key)
          .toList();

      // min/max are dataBindings — they define the data range, not appearance
      expect(bindingKeys,
          containsAll(['value', 'min', 'max', 'centerValue', 'innerValue']));
      expect(
          visualKeys,
          containsAll(
              ['sweepAngle', 'startAngle', 'tickCount', 'label', 'unit']));
      expect(
          layoutKeys,
          containsAll(
              ['width', 'height', 'padding', 'borderRadius', 'opacity']));
      expect(fontColorKeys, containsAll(['color', 'accent', 'fontSize']));
    });

    test(
        'T1.4: car_viz manifest contains core car visualization properties and category mappings',
        () {
      ...
      expect(metaByKey['doorLeft']?.category,
          equals(PropertyCategory.dataBindings));
      expect(metaByKey['doorRight']?.category,
          equals(PropertyCategory.dataBindings));
      expect(metaByKey['showRing']?.category, equals(PropertyCategory.visuals));
      expect(
          metaByKey['showLabels']?.category, equals(PropertyCategory.visuals));
      expect(metaByKey['fontSize']?.category,
          equals(PropertyCategory.fontsAndColors));
    });

    test(
        'T1.5: PropertyMeta specifies min, max, step bounds for numeric properties',
        () {
      ...
      final sweepAngleMeta =
          gaugeProps.firstWhere((p) => p.key == 'sweepAngle');
      expect(sweepAngleMeta.min,
          equals(10.0)); // min 10 ensures gauge is always visible
  ...
  group('Tier 2: Property Categorization Boundary & Corner Cases', () {
    ...
    test(
        'T2.2: Unknown widget kind returns empty categorized lists without throwing exception',
        () {
    ...
    test(
        'T2.4: Property keys within each of the 11 manifests are unique without duplicates',
        () {
      for (final kind in coreKinds) {
        final props = propertyManifest[kind]!;
    ...
    test(
        'T2.5: All 11 core widget manifests populate at least 3 out of 4 property categories',
        () {
      for (final kind in coreKinds) {
        final catMap = categorizedProperties(kind);
        final populatedCount =
            catMap.entries.where((e) => e.value.isNotEmpty).length;
