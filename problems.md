...
> **Status: FIXED** — Dashboard settings now includes a "Connection" section with Transport (Auto/Bluetooth/USB Serial), Auto-connect toggle, and Data rate throttle (5/10/20/50 Hz). `SettingsService` extended with `autoConnect` (bool) and `dataRate` (int Hz) fields with persistence. Dashboard `initState` checks `autoConnect` before connecting; `_onPayload` throttles ingestion by the configured rate. A manual "Connect" button appears in the toolbar when disconnected.
